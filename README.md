...
> **Built for the [Global AI Hackathon Series with Qwen Cloud](https://qwencloud-hackathon.devpost.com/) — Track 4: Autopilot Agent.** The scan → resolve → score → report → remediation pipeline runs as an agentic workflow: a Qwen orchestrator (OpenAI-compatible, model-swappable) plans connector dispatch, assists entity-resolution judgments, and writes the plain-language report — while safety gates run *before* any dispatch, every agent decision is written to the audit log, and a citation guard rejects any LLM claim that doesn't trace to a source finding. The audit log, encrypted PII vault, and safety floor are the production-readiness story most entries won't have.
