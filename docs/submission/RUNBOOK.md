...
- **Already done:** Workstreams A (public repo), B (Qwen integration, validated on Qwen Cloud), E (demo surface), D1 (architecture diagram), and the D asset drafts in `docs/submission/`.
- **Two open dependencies** (flagged ⚠ inline): `deploy.sh` is untested end-to-end, and there's no prod path yet to seed the judge demo account. Both close in the **deploy-hardening pass** (next item after this runbook).
...
> ⚠ **Before running this:** I do the deploy-hardening pass (validate `deploy.sh` + `docker-compose.prod.yml`, add the demo-account seed, confirm a synthetic source is enabled for the judge scan). Don't run Phase 1 until that's done — otherwise the first scan may produce no findings or judges can't log in.
...
```
Then in a browser: log in as the demo account, run a scan, open the report — confirm the score, the ✦ Qwen narrative with working citation chips, and "How Ayin ran this scan" all render. ⚠ The demo account + a finding-producing source come from the hardening pass.
...
- [ ] Demo account present + scannable (self-scan rule holds — synthetic fixtures or your own verified identifiers only)
...
| C — ECS deploy | ⏳ pending (hardening → provision → deploy) |
| Voucher | ⏳ pending (you) |
